...
export const filesSlice = createSlice({
  ...
  reducers: {
    ...
    moveEntry: (state, action) => {
      ...
      console.log(`Entry ${entryId} moved from ${oldParent.id} to ${newParent.id}`);
    },
    ...
    deleteEntry: (state, action) => {
      ...
      const parent = Object.values(state.tree).find((e) => e.isFolder && e.children.includes(entryId));
      if (parent) {
...
export const { setCurrentFile, updateFileContent, moveEntry, createEntry, deleteEntry, renameEntry } = filesSlice.actions;
